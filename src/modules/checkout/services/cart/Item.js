...
const {
  CONSTANTS
} = require('@evershop/evershop/src/lib/helpers');
const {
  buildUrl
} = require('@evershop/evershop/src/lib/router/buildUrl');
...
const {
  getSetting
} = require('../../../setting/services/setting');
const {
  getTaxPercent
} = require('../../../tax/services/getTaxPercent');
const {
  getTaxRates
} = require('../../../tax/services/getTaxRates');
const {
  calculateTaxAmount
    ...
    resolvers: [
      async function resolver() {
        ...
        if (sales_event) {
          ...
          const startDate = sales_event.start_date ?
            DateTime.fromJSDate(sales_event.start_date, {
              zone: 'UTC'
            }) :
            today.minus({
              days: 1
            });
          const endDate = sales_event.end_date ?
            DateTime.fromJSDate(sales_event.end_date, {
              zone: 'UTC'
            }) :
            today.plus({
              days: 1
          ...
          if (startDate < today && endDate > today) {
            return sales_event.discount_amount;
          } else {
            return 0;
          }
        }
