...
class ProductCollection {
  ...
  async init(args, { filters = [] }, { user }) {
    ...
    // seller_code filter
    const seller_codeFilter = filters.find((f) => f.key === 'seller_code');
    if (seller_codeFilter) {
      // Support like, equal and IN
      if (['LIKE', 'like'].includes(seller_codeFilter.operation)) {
        this.baseQuery.andWhere('product.seller_code', 'ILIKE', `%${seller_codeFilter.value}%`);
        currentFilters.push({
          key: 'seller_code',
          operation: 'like',
          value: seller_codeFilter.value
        });
      } else if (['IN', 'in'].includes(seller_codeFilter.operation)) {
        const values = seller_codeFilter.value
          .split(',')
          .map((v) => v.trim())
          .filter((v) => v.length > 0);
        if (values.length > 0) {
          this.baseQuery.andWhere('product.seller_code', 'IN', values);
          currentFilters.push({
            key: 'seller_code',
            operation: 'in',
            value: values.join(',')
        ...
        this.baseQuery.andWhere('product.seller_code', '=', seller_codeFilter.value);
        currentFilters.push({
          key: 'seller_code',
          operation: '=',
          value: seller_codeFilter.value
